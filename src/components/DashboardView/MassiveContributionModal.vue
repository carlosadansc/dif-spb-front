<template>
  <dialog class="modal" ref="massiveContributionModalRef">
    <div class="modal-box w-full max-w-[60vw]">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div>
          <button type="button" @click="addItem" class="btn btn-square bg-red-800 text-white w-full">
            ...
            <li v-for="(item, index) in contribution.productOrServices" :key="index"
              ...
              <div class="flex items-center gap-2">
                ...
                <div class="flex items-center gap-1 bg-gray-100 rounded-md px-2 py-1">
                  <!-- Botón decrementar -->
                  <button type="button" @click="decrementQuantity(index)" :disabled="item.quantity <= 1"
                    class="btn btn-xs btn-circle btn-ghost text-gray-600 hover:text-red-600 disabled:opacity-30">
                    <IconMinus class="w-3 h-3" />
                  </button>

                  <!-- Cantidad actual -->
                  <span class="text-sm font-bold text-gray-900 min-w-[2rem] text-center">{{ item.quantity }}</span>
        ...
        <div class="grid grid-cols-4 gap-4">
          <div>
            ...
            <input v-model="newBeneficiary.curp" v-maska="'@@@@######@@@@@@**'" type="text" placeholder="CURP"
              @input="newBeneficiary.curp = newBeneficiary.curp.toUpperCase()"
          ...
          <div>
            ...
            <input v-model="newBeneficiary.motherSurname" type="text" placeholder="Apellido Materno (opcional)"
              class="input input-sm mt-1 block w-full rounded-md border-gray-300" />
          </div>
        </div>

        <div class="grid grid-cols-3 gap-2">
          <button type="button" @click="addBeneficiary" class="btn btn-sm bg-blue-600 text-white">
          ...
          <ExportToExcelButton tooltip="Descargar plantilla Excel" @click="downloadTemplate" size="small"
            :require-admin="false" />
        </div>
        ...
        <div class="bg-gray-100 rounded-md p-5 max-h-64 overflow-y-auto">
          <h3 class="text-lg font-bold mb-2">Beneficiarios agregados ({{ beneficiaries.length }})</h3>
          <ul v-if="beneficiaries.length > 0">
            <li v-for="(ben, index) in beneficiaries" :key="index"
              class="flex items-center justify-between px-4 py-2 hover:bg-gray-50 border-b">
              <div class="flex-1">
                <span class="text-sm font-medium">{{ ben.name }} {{ ben.fatherSurname }} {{
                  ben.motherSurname }}</span>
                <span class="text-xs text-gray-500 ml-2">{{ ben.curp }}</span>
              </div>
  ...
  <!-- Confirm Clear Modal usando Teleport para evitar problemas de z-index -->
  <Teleport to="body">
...
import { IconPlus, IconTrash, IconFileSpreadsheet, IconMinus, IconX } from '@tabler/icons-vue';
...
// Data
const massiveContributionModalRef = ref();
...
watch(
  ...
  (value) => {
    if (value) {
      massiveContributionModalRef.value.showModal();
    } else {
      massiveContributionModalRef.value.close();
    }
  }
);
